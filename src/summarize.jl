...
function Base.convert(::Type{Array}, cs::Array{ChainDataFrame{NamedTuple{<:Any,V}},1}) where {V}
    T = promote_eltype_tuple_type(Base.tuple_type_tail(V))
    return convert(Array{T}, cs)
end
function Base.convert(::Type{Array{T}}, cs::Array{<:ChainDataFrame,1}) where {T}
