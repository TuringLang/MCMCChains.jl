...
@testset "Array constructor tests" begin
    ...
    @testset "Accuracy" begin
        ...
        long_val = vcat([val[:, :, i] for i = 1:nchains]...)
        squished_val = [val[:, :, i] for i = 1:nchains]
        chn = Chains(val)
        ...
        @test all(vcat([arr2[i] .== squished_val[i] for i = 1:nchains]...))
    end
    ...
    @testset "concretize" begin
        ...
        zz = [Any[0.5, 1, missing] for _ = 1:3]
        @test eltype(MCMCChains.concretize(zz)) === Vector{Union{Float64,Missing}}

        zzz = Any[Any[0.5, 1, missing] for _ = 1:3]
